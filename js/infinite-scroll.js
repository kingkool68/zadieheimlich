...
jQuery(document).ready(function($) {
	...
	var triggerOffset = $(document).height() - $triggerElement.offset().top; // The point of this is to do one calculation up front instead of multiple calculations every time the infinite scroll is triggered.
	var loadedScripts = [];
	var loadedStyles = [];
		...
		if( $(document).height() - triggerOffset < $(document).scrollTop() + $(window).height() ) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					// instantiate media element player for new video and audio elements injected on the page
